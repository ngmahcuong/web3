...
export const reportErrorMessage = (errorReporter: ErrorReporter, failure: number[]) => {
...
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types
export const getFailure = (t: TransactionReceipt) => {
  return t.logs
        ...
        provider
          ...
          .then((receipt) => {
            if (receipt) {
              ...
              if (failure) {
                if (!transaction?.hidePopup) {
                  addPopup(
                    {
                      type: 'error',
                      title: transactions[hash]?.summary,
                      message: reportErrorMessage(transaction?.errorReporter, failure)?.info,
                    },
                    transaction?.popupId,
                  );
                }
